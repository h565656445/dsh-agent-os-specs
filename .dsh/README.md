...
将 `.dsh` 目录复制到 `$DSH_HOME/.agent-presets/agent-os-specs/`：
...
重启 DeepSeek Harness 后，在会话预设列表选择 `Agent OS 规格文档专家`。

### 方式二：仅安装技能（skill）

将 SKILL.md 复制到 `$DSH_HOME/skills/dsh-agent-os-specs/`：

```powershell
$dst = Join-Path $env:DSH_HOME "skills\dsh-agent-os-specs"
New-Item -ItemType Directory -Force -Path $dst | Out-Null
Copy-Item ".\dsh\skills\dsh-agent-os-specs\SKILL.md" $dst -Force
```
